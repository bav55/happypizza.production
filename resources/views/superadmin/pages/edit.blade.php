...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ route('pages.edit', $page->id) }}">{{ $page->title }}</a></li>
    </ol>
    <div class="row">
        <div class="col-md-12">
            <div class="box box-warning">
                <div class="box-header with-border">
                    <h3 class="box-title">Редактирование {{ $page->title }}</h3>
                </div>
                        ...
                        <div class="col-md-9">
                            <div class="box-body pad">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title" class="form-control" value="{{ $page->title }}" required placeholder="Enter ...">
                                </div>
                                <label for="basic-url">ЧПУ (url)</label>
                                <div class="input-group{{ $errors->has('url') ? ' has-danger' : '' }}" style="margin-bottom: 10px;">
                                    <span class="input-group-addon" id="basic-addon3">http://happypizza.kz/</span>
                                    <input type="text" name="url" value="{{ $page->url }}" class="form-control" id="basic-url" aria-describedby="basic-addon3">
                                    @if ($errors->has('url'))
                                ...
                                <div class="form-group">
                                    <label>Превью</label>
                                    <textarea class="form-control" name="excerpt" rows="3" required placeholder="Enter ...">{{ $page->excerpt }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>Контент</label>
                                    <textarea id="content" class="textarea" name="content">{{ $page->content }}</textarea>
                                </div>
                                <hr>
                                <h3 class="box-title">SEO блок</h3>
                                <div class="form-group">
                                    <label>Заголовок</label>
                                    <input type="text" name="seo_title" class="form-control" value="{{ $page->seo_title }}" placeholder="Enter ...">
                                </div>
                                <div class="form-group">
                                    <label>description</label>
                                    <textarea name="seo_description" class="form-control" rows="2">{{ $page->seo_description }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>keywords</label>
                                    <textarea name="seo_keywords" class="form-control" rows="2">{{ $page->seo_keywords }}</textarea>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="box-body">
                                <a href="{{ route('pages.index') }}" type="button" class="btn btn-primary pull-left">Назад</a>
                                ...
                                @if ($page->image != null)
                                    <div class="form-group">
                                        <img src="{{ $page->image }}" class="img-responsive" id="preview-post-img">
                                    </div>
                                @else
                                    <div class="form-group">
